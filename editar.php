...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($nombre_completo) && !empty($direccion) && !empty($observacion)) {
        ...
        $sql = "CALL editar_tabla(?, ?, ?, ?)";
        
        // Preparar la sentencia para actualizar el registro
        if ($stmt = mysqli_prepare($conn, $sql)) {
            // Vincular los parámetros a la consulta (ID, nombre, dirección, observación)
            mysqli_stmt_bind_param($stmt, "isss", $id, $nombre_completo, $direccion, $observacion);
